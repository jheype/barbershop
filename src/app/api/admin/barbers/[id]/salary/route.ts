...
export async function POST(req: Request, ctx: Ctx) {
  ...
  try {
    ...
    const note = [`Salary (${barber.name})`, dateKey ? `Date: ${dateKey}` : null, noteIn || null].filter(Boolean).join(" • ");
    ...
    const salary = await prisma.barberSalaryPayment.create({
      data: {
        barberId: id,
        amountCents: cents,
    ...
    return NextResponse.json({ ok: true, salary, cashEntry });
  } catch (e) {
    console.error("POST /api/admin/barbers/[id]/salary FAILED:", e);
    return NextResponse.json({ error: "Error while recording salary." }, { status: 500 });
  }
}